...
@FunctionRecorder
public class YearControl extends DirectWriteEditor implements DataControl {
  ...
  private ValueInitializer widgetValue = (ValueInitializer)new WidgetValue();
  ...
  public void setWidgetValue(ValueInitializer paramValueInitializer) {
    this.widgetValue = paramValueInitializer;
  }
  ...
  public void createValueResult(CalculatorProvider paramCalculatorProvider, JSONObject paramJSONObject) {
    if (getWidgetValue() != null) {
      Object object = this.widgetValue.executeResult(paramCalculatorProvider);
  ...
  public void createValueResult(DataControl paramDataControl, Calculator paramCalculator, JSONObject paramJSONObject1, JSONObject paramJSONObject2) {
    if (getWidgetValue() != null) {
      ...
      try {
        object = this.widgetValue.executeResult((CalculatorProvider)paramCalculator);
      } catch (Exception exception) {
        exception.printStackTrace();
        ValueInitializer valueInitializer = getWidgetValue();
        Method[] arrayOfMethod = ValueInitializer.class.getMethods();
        boolean bool = true;
        Method method = null;
        for (Method method1 : arrayOfMethod) {
          if (method1.getName().equals("createAttrResult")) {
            method = method1;
            Class[] arrayOfClass = method1.getParameterTypes();
            if (method1.getParameterCount() > 2) {
              Class clazz = arrayOfClass[2];
              if (clazz.getName().equals("com.fr.json.JSONObject"))
                bool = false; 
            } 
          } 
        } 
        try {
          if (method != null)
            object = method.invoke(valueInitializer, new Object[] { paramCalculator, paramJSONObject1 }); 
  ...
  public boolean equals(Object paramObject) {
    return (paramObject instanceof YearControl && super.equals(paramObject) && ComparatorUtils.equals(this.startYear, ((YearControl)paramObject).startYear) && ComparatorUtils.equals(this.endYear, ((YearControl)paramObject).endYear));
  }
  ...
  public void readXML(XMLableReader paramXMLableReader) {
    ...
    if (paramXMLableReader.isChildNode()) {
      ...
      if (str.equals("YearAttr")) {
        ...
        this.widgetValue = (ValueInitializer)new WidgetValue();

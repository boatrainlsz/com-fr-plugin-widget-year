...
public class DYearEditorDefinePanel extends FieldEditorDefinePane<Year> {
  ...
  public DYearEditorDefinePanel() {
    initComponets();
  }
  
  protected void populateSubFieldEditorBean(Year paramYear) {
    this.directWriteCheckBox.setSelected(paramYear.isDirectEdit());
    this.waterMarkDictPane.populate((WaterMark)paramYear);
    populateStartEnd(paramYear);
  }
  
  protected Year updateSubFieldEditorBean() {
    Year year = new Year();
    year.setDirectEdit(this.directWriteCheckBox.isSelected());
    this.waterMarkDictPane.update((WaterMark)year);
    updateStartEnd(year);
    return year;
  }
  ...
  public void populateStartEnd(Year paramYear) {
    Formula formula1 = paramYear.getStartYearFM();
    Formula formula2 = paramYear.getEndYearFM();
    if (formula1 != null) {
      this.startDv.populate(formula1);
    } else {
      String str = paramYear.getStartText();
      this.startDv.populate(str);
    } 
    if (formula2 != null) {
      this.endDv.populate(formula2);
    } else {
      String str = paramYear.getEndText();
      this.endDv.populate(str);
    } 
  }
  
  public void updateStartEnd(Year paramYear) {
    ...
    if (object1 instanceof Formula) {
      ...
      paramYear.setStartYearFM(formula);
      paramYear.setStartText((String)null);
    } else {
      try {
        paramYear.setStartText((object1 == null) ? null : object1.toString());
    ...
    if (object2 instanceof Formula) {
      ...
      paramYear.setEndYearFM(formula);
      paramYear.setEndText((String)null);
    } else {
      try {
        paramYear.setEndText((object2 == null) ? null : object2.toString());
      } catch (ClassCastException classCastException) {}
    } 
  }
}

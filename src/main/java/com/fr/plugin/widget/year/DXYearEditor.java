...
public class DXYearEditor extends XDirectWriteEditor {
    private UITextField textField;

    private XWidgetCreator.LimpidButton btn;

    public DXYearEditor(YearControl paramYearControl, Dimension paramDimension) {
        super(paramYearControl, paramDimension);
    }

    public DXYearEditor(Year paramYear, Dimension paramDimension) {
        super(paramYear, paramDimension);
    }

    public CRPropertyDescriptor[] supportedDescriptor() throws IntrospectionException {
        String str = Inter.getLocText("Fine-Module_Year_Advanced");
        return (CRPropertyDescriptor[]) ArrayUtils.addAll((Object[]) super.supportedDescriptor(),
                (Object[]) new CRPropertyDescriptor[]{(new CRPropertyDescriptor("widgetValue", this.data.getClass()))
                        .setI18NName(Inter.getLocText("Fine-Module_Year_Value"))
                        .setEditorClass(YearWidgetValueEditor.class).
                        setPropertyChangeListener(new PropertyChangeAdapter() {
                    public void propertyChange() {
                        DXYearEditor.this.initFieldText();
                    }
    ...
    private void initFieldText() {
        Year year = (Year) this.data;
        if (year.getWidgetValue() != null) {
            WidgetValue widgetValue = year.getWidgetValue();
            Object object = widgetValue.getValue();
            if (object == null) {
                object = "";
                year.setWidgetValue(new WidgetValue(null));
            }
